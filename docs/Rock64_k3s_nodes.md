...
export SUBJECT='/C=US/ST=North Carolina/L=Cary/O=Uniblick.io/CN=uniblick.io'

openssl req -config /etc/ssl/openssl.cnf -newkey rsa:4096 -nodes -sha256 -keyout certs/domain.key -x509 -days 365 -out certs/domain.crt -subj "${SUBJECT}"

## Fedora
sudo cp certs/domain.crt /etc/pki/ca-trust/source/anchors/docker.uniblick.io.crt
...
## Ubuntu
sudo cp certs/domain.crt /usr/local/share/ca-certificates/docker.uniblick.io.crt
sudo update-ca-certificates
```

```bash
DOCKER_PASSWORD=$(date +%s | sha256sum | base64 | head -c 32 ; echo)

sudo apt install -y apache2-utils

echo "$DOCKER_PASSWORD" | htpasswd -Bic htpasswd registry

## ZDVmMmYwYzJkM2YwYWE5ZDhiOTdkZTU0
...
DOMAIN_CRT=$(cat certs/domain.crt | base64)
DOMAIN_KEY=$(cat certs/domain.key | base64)

cat > ci/registry-tls.yaml << EOF
apiVersion: v1
kind: Secret
metadata:
...
kubectl apply -f ci/registry-tls.yaml

kubectl apply -f ci/registry.yaml

kubectl create secret docker-registry regcred \
  --docker-server=docker.uniblick.io \
  --docker-username=registry \
  --docker-password=${DOCKER_PASSWORD} \
  --docker-email=xbcsmith@gmail.com
...
```bash
cfssl gencert -initca uniblick.json | cfssljson -bare uniblick
